from book_proj_mod.catalog import main
from book_proj_mod.catalog.models import Book, Publication

from flask import render_template, flash, request, redirect, url_for
from flask_login import login_required, current_user
from book_proj_mod import db
from book_proj_mod.catalog.forms import EditBookForm, CreateBookForm
...
def delete_book(book_id):
    ...
    if request.method == 'Post':
        db.delete.session(book)
        db.session.commit()
        flash(f'Book {book_id} with name {book.name} deleted successfully')
